...
def stylization(stylization_module, smoothing_module, content_image_path, style_image_path, content_seg_path,
                ...
                cuda, save_intermediate, no_post, label_remapping):
    # Load image
    ...
    # opencv_cont_pilimg = np.array(cont_pilimg)[:, :, ::-1].copy()
    # opencv_styl_pilimg = np.array(styl_pilimg)[:, :, ::-1].copy()
    # opencv_styl_pilimg = cv2.resize(opencv_styl_pilimg,(opencv_cont_pilimg.shape[1],opencv_cont_pilimg.shape[0]))
    # styl_seg_vis = cv2.resize(styl_seg_vis, (opencv_cont_pilimg.shape[1], opencv_cont_pilimg.shape[0]))
    # img_vis = np.concatenate((opencv_cont_pilimg,opencv_styl_pilimg),axis=1)
    # seg_vis = np.concatenate((cont_seg_vis,styl_seg_vis),axis=1)
    # final_vis = np.concatenate((img_vis,seg_vis),axis=0)
    # cv2.imwrite(output_image_path + "_vis.jpg", final_vis)
    # cv2.imshow('debug',final_vis)
    # cv2.waitKey(0)

    # new_cont_seg = np.zeros([cont_seg.shape[0],cont_seg.shape[1],3], dtype=np.uint8)
    # new_styl_seg = np.zeros([styl_seg.shape[0], styl_seg.shape[1], 3], dtype=np.uint8)
    # for i in range(0,3):
    #     new_cont_seg[:, :, i] = cont_seg
    #     new_styl_seg[:, :, i] = styl_seg
    # cont_seg = new_cont_seg
    # styl_seg = new_styl_seg
    ...
    return cont_seg_vis, styl_seg_vis